...
public:
	...
	double Eval(const Eigen::Vector3d& _x)
	{
		...
		double sdf_value = pow(_x.x() - m_center.x(), 2) + pow(_x.y() - m_center.y(), 2) + pow(_x.z() - m_center.z(), 2)
		        - pow(m_radius, 2);
		return sdf_value;
	}
...
public:
	...
	double Eval(const Eigen::Vector3d& _x)
	{
		...
		double first_part = pow(pow(_x.x() - m_center.x(), 2)
		        + pow(_x.y() - m_center.y(), 2) + pow(_x.z() - m_center.z(), 2)
		        + pow(m_radius, 2) -  pow(m_a, 2) , 2);
		double second_part = 4 * pow(m_radius, 2) *
		        ((pow(_x.x() - m_center.x(), 2)) + (pow(_x.y() - m_center.y(), 2)));
        double sdf_value = first_part - second_part;
		return sdf_value;
	}
